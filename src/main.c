...
int main(int argc, char *argv[]) {
    ...
    if (is_iso(argv[1]) && is_iso(argv[2])) {
        ...
        if (pid == 0) {
            // Child process
            int ret = execlp(
                "7z", "7z", "x", "-aoa", "-bb0", "-bd", "-y", outArg, "--", argv[2], "Audio", NULL);
            assert(ret == 0);
            _exit(EXIT_SUCCESS);
        } else if (pid < 0) {
            // Fork failed
    ...
    for (unsigned long groupIndex = 0;
         ...
         groupIndex++) {
        UnshieldFileGroup *group = unshield_file_group_find(unshield, FILE_GROUPS[groupIndex]);
        if (!group) {
        ...
        for (unsigned long i = group->first_file; i <= group->last_file; i++) {
            if (unshield_file_is_valid(unshield, (int)i)) {
                ...
                if (groupIndex == 0 && !strcmp(dir, "audio")) { // Keep casing consistent
                    dir[0] = 'A';
                }
                int ret = sprintf(targetDir, "%s%s%s", outputDir, dir ? "/" : "", dir ? dir : "");
                assert(ret > 0);
